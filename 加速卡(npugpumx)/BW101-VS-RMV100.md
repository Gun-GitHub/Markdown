...
| :----------------------------: | :--------------------------: | :---------------------: | :--------------------------------------------------: |
|               厂商               |              海光              |          天数智芯片          |                                                      |
|         是否存在 fp64 计算模块         |            厂商未说明             |          厂商未说明          |                                                      |
|         是否存在 fp32 计算模块         |              存在              |           存在            |                                                      |
|              显存频率              |        ***1800 MHz***        |        1600 MHz         |                                                      |
|           核心计算时峰值频率            |           1350 MHz           |     ***1600 MHz***      |                                                      |
|              显存大小              |         ***65504***          |          32768          |                                                      |
|      与cpu相比 fp64 计算误差最大值       | ***0.00000000000090949470*** | 0.00875667685090775194  | 可能是 RM-V100 未进行三角函数相关计算导致的,***后经过测试 普通四则运算差距也是这么大*** |
|      与cpu相比 fp32 计算误差最大值       |    0.00118991340650609345    | 0.00088450437351639266  |                                                      |
|      与cpu相比 fp64 计算误差平均值       | ***0.00000000000010842882*** | 0.00412309574948392307  | 可能是 RM-V100 未进行三角函数相关计算导致的,***后经过测试 普通四则运算差距也是这么大*** |
|      与cpu相比 fp32 计算误差最大值       |    0.00014915226851994136    | 0.00014795610787007263  |                                                      |
|         是否支持 fp64 四则运算         |              支持              |           支持            |                                                      |
|      是否支持 fp64 三角函数等特殊运算       |           ***支持***           |           不支持           |            在测试用例中,涉及三角函数特殊预算 fp64返回结果全为 0            |
|         是否支持 fp32 四则运算         |              支持              |           支持            |                                                      |
|      是否支持 fp32 三角函数等特殊运算       |              支持              |           支持            |                                                      |
| fp64 带三角函数等特殊运算一次计算平均时间(单位:ms) | ***1.89906299114227294922*** | 20.98007965087890625000 |     RM-V100 虽然不支持 fp64 三角函数特殊运算,但是它不会报错,继续运算,不会停     |
| fp32 带三角函数等特殊运算一次计算平均时间(单位:ms) |    0.75064218044281005859    | 0.60118657350540161133  |                                                      |
| 带三角函数等特殊运算一次计算平均时间 fp64 : fp32 | ***2.52744412422180175781*** | 34.6879997253417968750  |                                                      |
|   fp64 普通四则运算一次计算平均时间(单位:ms)   | ***0.07398736476898193359*** | 1.50069546699523925781  |                                                      |
|   fp32 普通四则运算一次计算平均时间(单位:ms)   |    0.04850733280181884766    | 0.04620220884680747986  |                                                      |
|   普通四则运算一次计算平均时间 fp64 : fp32   | ***1.52528202533721923828*** | 32.48103332519531250000 |                                                      |
|        fp32 Mean GFLOPS        |            43549             |         51526.3         |                        **存疑**                        |
|        fp64 Mean GFLOPS        |           43394.9            |         961.63          |                        **存疑**                        |
### ***结论:***
 1. BW101 的计算精度在 fp64 远高于 RM-V100
 2. BW101 支持更多的在 fp64 的三角函数等特殊运算
 3. BW101 在 fp64 计算速度远快于 RM-V100
 4. BW101 ***显存大小是*** RM-V100 的 ***2倍***
 5. RM-V100 直接支持 CUDA, BW101 需要学习 hip_runtime
## ***总结:***
BW101 在 fp64 计算上表现更好更稳定, 显存是 RM-V100 的2倍,但使用它需要一定量的学习, 从 CUDA 迁移过来有一定的成本, 其他层面上相差不大
